...
// sphere.xyz 是中心位置
// sphere.w 是半径
float DistanceBySphere(float3 p, float4 sphere)
{
...
// 圆柱距离
float DistanceByCylinder(float3 p, float3 a, float3 b, float r)
{
    float3 ap = p - a;
    float3 ab = b - a;
    float t = dot(ap, ab) / dot(ab, ab);
...
float3 GetNormalDemo(float3 p)
{
    ...
    float3 n = DistanceBySphere(p, sphere) - float3(
        DistanceBySphere(p - e.xyy, sphere),
        DistanceBySphere(p - e.yxy, sphere),
        DistanceBySphere(p - e.yyx, sphere)
    );
    return normalize(n);
}
...
{
    float h = saturate((b - a) / k * 0.5 + 0.5);
