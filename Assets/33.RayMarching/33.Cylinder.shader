Shader "Unlit/33.Cylinder"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                return min(DistanceByCylinder(p, float3(-1, 1, 0), float3(1, 0, 0), 0.5), p.y);
            }
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                if (d < MAX_DISTANCE)
                {
                    ...
                    col.rgb = ndotl + ndoth;
                    // 阴影
                    float sd = Raycast(p + n * MIN_DISTANCE * 2, lDirWS);
                    if (sd < MAX_DISTANCE) col.rgb *= 0.5; 
                }
                else discard;
