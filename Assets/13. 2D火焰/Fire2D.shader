    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                half time;
                Unity_Multiply_half(_Time.y, -_UpSpeed, time);
                Unity_TilingAndOffset_half(o.uv, half2(1.,1.), half2(0.,time), o.noiseUV);
                return o;
            }
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                // 噪声
                Unity_SimpleNoise_half(i.noiseUV, _NoiseScale, simpleNoise);
                simpleNoise -= 0.5;
                simpleNoise *= _DistortionStrength;

                // 对底影响
                Unity_Power_half(i.uv.y, _FireBottomOffset, fireBottomOffset);

                // 扰动UV
                fireUVOffset = half2(0., fireBottomOffset * simpleNoise);
                Unity_TilingAndOffset_half(i.uv, half2(1,1), fireUVOffset, fireUV);

                // 上焰
                fireUp = pow(1.0 - fireUV.y, 2.);
                
                // 内焰
                Unity_Ellipse_half(half2(fireUV.x, fireUp), _ElliSize.x, _ElliSize.y, innerFire);

                // 层次
                postersize = (fireUp + 0.4) * 1.5;
                Unity_Posterize_half(postersize, 4, postersize);
                innerFire += postersize;
                col = innerFire * _FireColor;

                // 外焰
                Unity_Ellipse_half(half2(fireUV.x, fireUp), _ElliSize.z, _ElliSize.w, outerFire);
